...
data       = "data/topologia.txt" #The data of the n-simulation where continuously written on this archive.
N          = 200  
start_node = 0

#como agregamos inmunes en vez de usar "data/evolution.txt", usamos "data/episize.txt""
simulation = np.loadtxt("data/episize.txt",usecols=0, ndmin=1, dtype=int) #size of every epidemic in data. 
TotalSimu  = 150
...
for n_simul in range(TotalSimu):
	...
	if Grafico:
		if (p_distribution.size>2):
			...
			plt.ylim(0,y.max()+5)
			plt.plot(x,y)
			plt.savefig("network/epidemic%i_node%i.png" %(n_simul,start_node))
			plt.clf()
			...
			plt.legend()
			plt.savefig("network/degree_histogram%i_node%i.png" %(n_simul,start_node))
			plt.clf()
			...
			if (info and n_simul>-1):
				...
				nx.draw(DG, **args_g)
				nx.draw_networkx_nodes(G, pos, nodelist=[start_node], node_color= "r", node_size=25)
				plt.savefig("network/grafo%i_node%i.png" %(n_simul,start_node))
				#plt.show()
				plt.clf()
				#plt.show()
...
plt.savefig("network/Histograma_promedio_node%i.png"%start_node)
plt.show()
plt.clf()
print()
