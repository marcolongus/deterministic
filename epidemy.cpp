//Versión del programa donde todas las partículas tienen la misma condición inicial.
//La evolución espacial es al azar.
//La evolución de la dinámica epidémica también.
#include <bits/stdc++.h>
...
int main(void){
	...
	ofstream episize("data/episize.txt");
	ofstream simtxt("data/sim.txt");

	cout   << "Semilla dinamica espacial: "  << seed_space	<< endl;
	cout   << "Semilla dinamica sir     : "  << seed_sir    << endl;

	simtxt << "Semilla dinamica espacial: "  << seed_space		 << endl;
	simtxt << "Semilla dinamica sir     : "  << seed_sir    	 << endl;
	simtxt << "N 						: "  << N				 << endl;
	simtxt << "L    					: "  << L   			 << endl;
	simtxt << "Active Speed				: "  << active_velocity  << endl;

	#pragma omp parallel for
    for (int n_simul=0; n_simul<150; n_simul++){
    	cout <<"--------------------------------------" << endl;
	    ...
	    for (int i=0; i<N; i++){
	        ...
	        if(i>0 and i < n_inmunes) A.set_refractary();
	    ...
	    forn(i,0,N){
	    	if (system[i].is_infected()) {cout << "Particula: " <<  i  << "  Vel: " << system[i].velocity << endl; first_set.insert(i);}
	    }
	    ...
	    bool directed_inmunization = false;
	    ...
	    double time=time_i;
	    int contador3 = 0; //Cantidad de whiles
	    /****************************************************************************************************************/
		...
		while(num_infected>0){
				...
				if ( (contador3 % 10001) == 0 ) {cout << time << endl;}
	            ...
	            forn(i,0,N){
	                if ( contador3 %  100 == 0 ) bandera = true; //si pongo false apago la animación. 
                	if (bandera){
                    	anim << system_new[i].x << " ";
                    	anim << system_new[i].y << " ";
                    	anim << time            << " ";
	            ...
	            system=system_new; //actualizacion del sistema.
	            forn(i,0,N){
