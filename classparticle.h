...
const double L = 70;                 //Lado del cuadrado
...
int n_inmunes = 1 + 0;            // Cantidad de inmunes
...
const double p_dinit = 0.02;  //población inicial de refractarios dirigidos.
...
mt19937::result_type seed_space = time(0);
mt19937 gen_space(seed_space);                             //Standard mersenne_twister_engine seeded time(0)
uniform_real_distribution<double> dis_space(0., 1.); // dis(gen), número aleatorio real entre 0 y 1.
...
class particle{
    ...
    particle(double x1, double y1, double vel, double ang, double ref_t);
...
particle::particle(double x1, double y1, double vel, double ang, double ref_t){
    x = x1;
    y = y1;
    velocity = vel;
    angle = ang;
...
particle create_particle(void){
    ...
    //velocity= pow( dis_icond(gen_icond)*( pow(v_max,1-k_powerl) - pow(v_min,1-k_powerl))+pow(v_min,1-k_powerl), 1./(1.-k_powerl)); //power_law
    //velocity=active_velocity;

    particle A(x,y,velocity,angle,0);
...
particle evolution(vector<particle> &system, vector<int> &index, bool inter){
    ...
    if (dis_space(gen_space)<p_rotation) {
        double ruido=(dis_space(gen_space)-0.5)*Pi;
        A.angle+=ruido;
    }
    ...
    if (A.is_infected() && flag){ 
    	if(A.refract_time > 200){A.set_refractary();}
    	else{A.refract_time+=delta_time;}
    }
